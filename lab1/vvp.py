...
xnew = np.linspace(1989., 2019., 100)
x_ = np.linspace(1989., 2016., 100)

fnew = [qsn(i,qs_coeff,year,GPD_VVP) for i in xnew]
df = [dqs(i,qs_coeff,year,GPD_VVP) for i in xnew]
y_lagr=[l.L(i,year,GPD_VVP) for i in x_]

fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(10, 10), dpi=100)
axes[0].plot(xnew, fnew, color="red", linewidth=1.0, linestyle="solid",label="S(x)")
...
axes[1].plot(xnew, df, color="red", linewidth=1.0, linestyle="solid",label="dS")
...
axes.plot(x_, y_lagr, color="red", linewidth=1.0, linestyle="solid",label="L(x)")
